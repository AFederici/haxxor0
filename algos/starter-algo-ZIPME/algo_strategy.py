...
class AlgoStrategy(gamelib.AlgoCore):
    ...
    def on_turn(self, turn_state):
        ...
        gamelib.debug_write("HEALTHS (ME FIRST) " , game_state.my_health, game_state.enemy_health)
    ...
    def deploy_attackers(self, game_state):
        ...
        while game_state.get_resource(game_state.BITS) >= game_state.type_cost(SCRAMBLER) and len(deploy_locations) > 0:
            ...
            path = gamelib.navigation.ShortestPathFinder()
            #change this to be exclusively the opposite side cause itll never be anything else
            actual_path = path.navigate_multiple_endpoints(deploy_location, enemy_edges, game_state)
            if counter == 0:
                gamelib.debug_write("START -------- " , deploy_location)
                gamelib.debug_write("PATH -------- " , actual_path)
